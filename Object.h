...
{
private:
	// 높이 맵 이미지 픽셀(8-비트)들의 이차원 배열이다. 각 픽셀은 0~255의 값을 가진다.
...
public:
	CHeightMap(LPCTSTR pFileName, int nWidth, int nLenght, D3DXVECTOR3 d3dscScale);
	~CHeightMap(void);
...
public:
	...
	float GetHeight(float x, float z, bool bReverseQuad = false){
		return(m_pHeightMap->GetHeight(x, z, bReverseQuad) * m_d3dxvScale.y);}
