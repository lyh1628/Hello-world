...
// 이 함수는 지형의 (x, z)좌표가 주어질 때 높이 맵 (x, z)좌표에서 지형의 법선 벡터를 계산한다.
...
float CHeightMap::GetHeight(float fx, float fz, bool bReverseQuad)
{
	...
	if (bReverseQuad)
	{
		...
		fzPercent < fxPercent인 경우. TopLeft의 픽셀 값은 (fTopLeft = fTopRight + (fBottomLeft - fBottomRight))로 근사한다.
		fzPercent > fxPercent인 경우, BottomRight의 픽셀 값은 (fBottomRight = fBottomLeft +(fTopRight - fTopLeft))로 근사한다.*/
	...
	else
	{
		...
		(fzPercent < (1.0f - fxPercent))인 경우 TopRight의 픽셀 값은 (fTopRight = fTopLeft + (fBottomRight - fBottomLeft))로 근사한다.
		(fzPercent ≥ (1.0f - fxPercent))인 경우 BottomLeft의 픽셀 값은 (fBottomLeft = fTopLeft + (fBottomRight - fTopRight))로 근사한다.*/ 
		if (fzPercent >= fxPercent)
			fBottomRight = fBottomLeft + (fTopRight - fTopLeft);
		else
			fTopLeft = fTopRight + (fBottomLeft - fBottomRight);
	}
	// 사각형의 네 점을 보간하여 높이( 픽셀 값)을 계산한다.
...
CHeightMapTerrain::CHeightMapTerrain(ID3D11Device* pd3dDevice, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, D3DXVECTOR3 d3dxvScale, D3DXCOLOR d3dxColor) : CGameObject(0)
{
	...
	cwQuadsPerBlock, czQuadPerBlock은 격자 메쉬의 가로 방향과 세로 방향 사각형의 개수 이다.*/
	...
	// 지형에서 가로, 세로 방향으로 격자 메쉬가 몇 개가 있는 가를 나타낸다.
