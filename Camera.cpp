...
{
	// 카메라의 위치와 방향이 바뀌면 카메라 변환 행렬을 다시 계산한다.
	//카메라의 z-축 벡터를 정규화한다.
...
//		__in const D3DXVECTOR3 *pvPosition,	//카메라의 위치 벡터
//		__in const D3DXVECTOR3 *pvLookAt, 	//카메라가 바라보는 점의 위치 벡터
//		__in const D3DXVECTOR3 *pvUp 	//카메라의 Up 벡터(보통 (0, 1, 0)을 사용)
//		);
//	
//	vLook = normal(vLookAt - vPosition)
//	vRight = normal(cross(vUp, vLook))
...
// 이 함수는 바운딩 박스(최소점과 최대점)가 절두체의 어떤 면의 바깥쪽에 있으면 false를 반환하고 그렇지 않으면 true를 반환한다.
